...
//        });
        ...
        var shareList = function () {
            ...
            var itemSideLength = $(window).width()*0.7/ cr.col;
            $("body ul.shareList").remove();
            $("body div.cover").remove();
            var list = $('<ul class="shareList"></ul>');
            //list.append('<div class="shareListTitle" style="position: initial;">分享到:</div>');
            ...
            var rowHeight=itemSideLength*0.9;

            list.css({
                //height: rowHeight * cr.row
            });
            list.find("li").css({
                //height: rowHeight
                width: itemSideLength,
                height: 80
            ...
            var showList = function () {
                ...
                var $listWrapper=$('<div class="in-share-wrapper"/>');
                $listWrapper.append('<div class="in-share-title">分享到:</div>');
                $listWrapper.append(list);
                $('body').append($listWrapper);
            }
            //close
            var closeList = function (callback) {
                var pendingRemoveItems=[$(".cover"),$(".in-share-wrapper"),list];
                pendingRemoveItems.forEach(function (item) {
                    item.addClass('remove');
                });
                //list.addClass('remove');
                //$(".cover").addClass('remove');
                //$(".cover").find('.in-share-wrapper').addClass('remove');
                setTimeout(function () {
                    pendingRemoveItems.forEach(function (item) {
    ...
    $.fn.umshare.share = function (plat, c) {
        ...
        var shareToPlat = {
            'wx_session': function () {
                shareViaWechat(Wechat.Scene.TIMELINE, c.data.content.subject, c.data.content.text, c.data.content.furl, c.data.content.img, c.data.content.shareType);
            },
            'wx_timeline': function () {
                shareViaWechat(Wechat.Scene.SESSION, c.data.content.subject, c.data.content.text, c.data.content.furl, c.data.content.img, c.data.content.shareType)
            },
            'sina': function () {
                sharePanel(plat, c);
            },
    ...
    var shareViaWechat = function (scene, title, desc, url, thumb, type) {
        if (!url) {
            alert('分享链接不能为空');
            return;
        }
        ...
        var msg = {
            ...
            description: desc ? desc : "share content",
            url: url ? url : "",
            thumbData: !!thumb ? thumb : null
        };
        ...
        Wechat.share({'scene': scene, text: '', message: msg}, function () {
//                 alert('分享成功');
        }, function (res) {
//                 alert('分享失败');
        });
    ...
    $.fn.umshare.defaults = {
        'data': {
            'content': {
                'text': '让移动应用快速整合社交分享功能！'
            },
            'platform': {
                'wx_session': {
                    'name': '微信好友',
                    'content': {
                        'text': '让移动应用快速整合社交分享功能！发给微信好友'
                    }
                },
                'wx_timeline': {
                    'name': '朋友圈',
                    'content': {
                        'text': '让移动应用快速整合社交分享功能！发给微信朋友圈'
                    }
                }
                //'sina': {
                //    'name': '新浪微博',
                //    'content': {
                //        'text': '让移动应用快速整合社交分享功能！发给新浪微博'
                //    }
                //},
                //'tencent': {
                //    'name': '腾讯微博',
                //    'content': {
                //        'text': '让移动应用快速整合社交分享功能！发给腾讯微博'
                //    }
                //},
//        'renren':{
//            'name':'人人网',
//            'content': {
//                'text' : '让移动应用快速整合社交分享功能！发给人人网'
//            }
//        },
//        'douban':{
//            'name':'豆瓣',
//            'content': {
//                'text' : '让移动应用快速整合社交分享功能！发给豆瓣'
//            }
//        }
//      'sms':{
//          'name':'短信',
